...
Only matching high-confidence promoted entries become typed edges. Explicit
`defer` entries remain encrypted quarantine records but count as reviewed
deferrals instead of open review work. Missing, duplicate, wrong-type, or
schema-invalid entries do not silently promote ambiguous data; they remain
quarantined or fail the run before write.
...
| `npm run logseq:semantic-inventory` | Yes, when pointed at a private local graph | No | Emits a plaintext-free inventory of schema-known property keys, hashed unknown keys, endpoint type counts, refs, dates, asset references, and topic-candidate source counts from tags/hash tags without promoting them. |
...
| `npm run logseq:semantic-local` | Yes, when pointed at a private local graph | No | Converts a bounded source window, promotes typed endpoints, high-confidence explicit wikilink property edges, exact non-wikilink targets that uniquely match a typed endpoint title or explicit alias of the required type in the same batch, and schema-safe exact suffix-tag edges, quarantines ambiguous relationship suffixes and unresolved non-wikilink relationship targets for encrypted review, runs local CRUD/leakage proof, and records a local-only ledger entry with Cloudflare sync explicitly paused. |
...
| `npm run logseq:semantic-topic-review-resolution-draft` | Topic packet only | No | Requires explicit acknowledgement and writes a private all-defer topic resolution map outside the repo. It terminally accounts for unreviewed candidates without promoting topic nodes. |
| `npm run logseq:semantic-topic-review-local` | Topic packet/resolution map only | No | Requires explicit acknowledgement, unlocked local keyring, and local graph directory. Writes promoted topics as encrypted local-private objects and terminal defer/reject decisions as encrypted quarantine objects, then emits a plaintext-free local ledger. |
...
high-confidence explicit wikilink property edges, exact non-wikilink property
targets that uniquely match a same-batch typed endpoint title or explicit alias
of the required type, and schema-safe exact suffix tags. It records ambiguous relationship
suffixes and unresolved or wrong-type non-wikilink relationship targets as
encrypted quarantine candidates, runs local CRUD/leakage proof, and records a ledger entry with
`sync.attempted=false`. Local-only mode fails fast if stale live sync
...
Use `logseq:semantic-review-packet` only for private local review work. It
...
Use `logseq:semantic-topic-review-packet` only for private local topic curation.
It requires
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_PACKET_ACK=write-local-private-topic-review-packet`
and `LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_PACKET_PATH`, refuses to write inside the
repository checkout, and may include plaintext tag/topic candidate values in the
packet file. The command scans the selected semantic source mode, groups plain
...
Use `logseq:semantic-topic-review-resolution-draft` when no human-reviewed topic
map exists and the safe action is to withhold all tag-derived candidates. It
requires
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_RESOLUTION_DRAFT_ACK=write-local-private-topic-review-resolution-draft`,
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_PACKET_PATH`, and
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_RESOLUTION_PATH`. The generated map contains
only high-confidence `defer` decisions, writes outside the repository, prints
counts only, and does not create topics. Edit that private map manually before
import if some candidates are truly controlled topics.

Use `logseq:semantic-topic-review-local` only after the topic review report is
clean. It requires
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_IMPORT_ACK=write-encrypted-local-topic-review-objects`,
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_PACKET_PATH`,
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_RESOLUTION_PATH`,
...
`LIVING_ATLAS_LOGSEQ_TOPIC_REVIEW_LEDGER_PATH` to write the plaintext-free
ledger outside the repository. It writes promoted topics as encrypted
`local-private` endpoint objects, writes terminal `defer`/`reject` decisions as
encrypted `quarantine` review records, and never attempts Cloudflare sync.
Existing topic review objects are left unchanged by default; set

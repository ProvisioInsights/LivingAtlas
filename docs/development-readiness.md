...
| `npm run logseq:semantic-local` | Yes, when pointed at a private local graph | No | Converts a bounded source window, promotes typed endpoints, high-confidence explicit wikilink property edges, and schema-safe exact suffix-tag edges, quarantines ambiguous relationship suffixes and non-wikilink relationship targets for encrypted review, runs local CRUD/leakage proof, and records a local-only ledger entry with Cloudflare sync explicitly paused. |
...
high-confidence explicit wikilink property edges and schema-safe exact suffix
tags, records ambiguous relationship suffixes and non-wikilink relationship
targets as encrypted quarantine candidates, runs local CRUD/leakage proof, and records a ledger entry with

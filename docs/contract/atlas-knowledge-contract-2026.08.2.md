# Atlas Knowledge Contract — revision 2026.08.2

Status: Published
Protocol revision: MCP `2026-07-28` (only)
Plane: consumer
Schemas: `packages/atlas-contract/schema/2026.08.2/`

## 0. What changed in 2026.08.2

A **compatible (additive)** revision. A consumer pinned to 2026.08.1 needs no code
change; a consumer that adopts this one gains one outcome it can branch on.

| change | policy |
|---|---|
| `carried-as-assertion` added to the `error_code` vocabulary | §11.1 **A5** — open vocabulary, and §11.3: `x-atlas-known-values` is a hint, never a whitelist |
| `carried-as-assertion` added to the `outcome` enum of `atlas.entity.resolve.v1` | §11.1 **A4** — additive to an output closed enum, subject to [C-06] |

**What it means.** An identifier resolves, and what it names is an ASSERTION rather
than an entity. The old store gave edges their own object ids; an edge became a
claim, not a thing, so the id has no entity to resolve to — and answering with one
of the edge's endpoints would silently turn every reference to the edge into a
reference to something it merely mentioned. `resolve()` therefore refuses, and the
refusal names the assertion. Refusing is correct for an entity resolver; refusing
without saying where the content went would not be.

**Servers on 2026.08.1 are unaffected and remain correct.** Per [C-06] they deliver
this outcome as `other`, with the precise `code` carried in `atlas.error:v1` — which
consumers already **MUST** [C-05] tolerate whether or not they have seen it before.
...
|---|---|
| `packages/atlas-contract/schema/2026.08.2/` | **shape** — what fields exist, what types they carry, what is required |
...
Revision 2026.08.2 changes nothing about the twelve tools, the record shapes, the
limits, or the protocol. It changes exactly one thing: the vocabulary those tools
carry. The predicate hint published in `x-atlas-known-values` is the only byte-level
difference from 2026.08.0.
...
A revision is cited as the pair `atlas-knowledge-contract` + `2026.08.2`. That pair
identifies both artifacts, because they ship together.

- **A specific schema** is cited by its `$id`, e.g.
  `urn:living-atlas:contract:2026.08.2:tool:atlas.assertion.query.v1:output`.
...
again, and the bytes of `schema/2026.08.2/` will not either once it is frozen.

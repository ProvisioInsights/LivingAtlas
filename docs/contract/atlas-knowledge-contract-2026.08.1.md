# Atlas Knowledge Contract — revision 2026.08.1

Status: Published
Protocol revision: MCP `2026-07-28` (only)
Plane: consumer
Schemas: `packages/atlas-contract/schema/2026.08.1/`
...
|---|---|
| `packages/atlas-contract/schema/2026.08.1/` | **shape** — what fields exist, what types they carry, what is required |
...
Revision 2026.08.1 changes nothing about the twelve tools, the record shapes, the
...
A revision is cited as the pair `atlas-knowledge-contract` + `2026.08.1`. That pair
identifies both artifacts, because they ship together.

- **A specific schema** is cited by its `$id`, e.g.
  `urn:living-atlas:contract:2026.08.1:tool:atlas.assertion.query.v1:output`.
...
again, and the bytes of `schema/2026.08.1/` will not either once it is frozen.

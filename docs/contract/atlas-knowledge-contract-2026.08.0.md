# Atlas Knowledge Contract — revision 2026.08.0

Status: Published
Protocol revision: MCP `2026-07-28` (only)
Plane: consumer
Schemas: `packages/atlas-contract/schema/2026.08.0/`
...
|---|---|
| `packages/atlas-contract/schema/2026.08.0/` | **shape** — what fields exist, what types they carry, what is required |
...
A revision is cited as the pair `atlas-knowledge-contract` + `2026.08.0`. That pair
identifies both artifacts, because they ship together.

- **A specific schema** is cited by its `$id`, e.g.
  `urn:living-atlas:contract:2026.08.0:tool:atlas.assertion.query.v1:output`.
...
ship as a new revision directory; the bytes of `schema/2026.08.0/` never change again.

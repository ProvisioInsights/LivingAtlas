# Atlas Knowledge Contract — revision 2026.08.4

Status: Published
Protocol revision: MCP `2026-07-28` (only)
Plane: consumer
Schemas: `packages/atlas-contract/schema/2026.08.4/`

## 0. What changed in 2026.08.4

A **compatible** revision that changes packaging, not meaning. A consumer pinned to
2026.08.3 needs no code change and loses nothing: the same fourteen tools, the same
arguments, the same results, the same limits, the same record shapes. What changes
is that every published TOOL document is now **self-contained** — the shared
definitions and record documents its `$ref`s reach are embedded under the
document's own `$defs`, each copy keeping its original `$id`. No `$ref` string
changed; a `urn:` reference now resolves to the embedded resource, which is the
standard JSON Schema 2020-12 bundling process.

| change | policy |
|---|---|
| tool documents bundle their `$ref` closure under `$defs` | §11.1 **A11** — bundling; no `$ref` string changes and no payload validates differently |

**Why.** MCP protocol revision `2026-07-28` ("$ref Resolution") forbids a client
from dereferencing anything outside the schema document it was handed: each
`tools/list` entry is compiled in isolation, and an unresolved external `$ref` is a
rejection, not a skip. 2026.08.3's tool documents referenced the shared and record
documents by URN without carrying them, so they were unresolvable BY CONSTRUCTION
for every conforming client — the MCP SDK's own client refused every consumer-plane
call at compile time (`can't resolve reference … from id …`) while the server,
holding the full corpus, validated the same bytes happily. A contract whose
published documents cannot be compiled by the clients the protocol defines is not
published in any useful sense; this revision repairs that.

**What is deliberately unchanged.**

- **The URN scheme keeps its security property.** Every `$ref` still names a
  `urn:`, which has no retrieval semantics — nothing is fetchable over a network,
  embedded or not. Bundling strengthens the local-only-resolution guarantee rather
  than trading it away.
- **The standalone `common.*` and `records/*` documents are still published** and
  are still the reviewed source of shape truth. An embedded copy is exactly the
  published definition — a generator assertion and a test hold the two identical —
  so a reader of either copy is reading the same artifact.
- **Closure is computed per tool.** A tool document embeds what its `$ref`s
  transitively reach and nothing else, for the same reason the shared documents
  publish only what each wire side can reference: an embedded definition nothing
  reaches is published, reviewed by nobody, and reads as authoritative to whoever
  finds it.

**Servers on 2026.08.3 are unaffected and remain correct** in what they accept and
return; what their published tool documents cannot do is compile in a conforming
client's validator, which is the defect this revision exists to remove.
...
|---|---|
| `packages/atlas-contract/schema/2026.08.4/` | **shape** — what fields exist, what types they carry, what is required |
...
Fourteen tools, one plane. Operational capabilities — migration windows, tiering
...
| `atlas.entity.create.v1` | mint a new entity — the id is chosen by Atlas, never derived, never deduplicated |
| `atlas.entity.rename.v1` | change what an entity is called, never what it is |
...
Revision 2026.08.4 changes nothing about the record shapes, the limits, the
protocol, or the fourteen tools 2026.08.3 published — it changes only how each
tool document is packaged (§0). The vocabulary those tools carry is unchanged
from 2026.08.2, whose own only byte-level difference from 2026.08.0 was the
predicate hint published in `x-atlas-known-values`.
...
A revision is cited as the pair `atlas-knowledge-contract` + `2026.08.4`. That pair
identifies both artifacts, because they ship together.

- **A specific schema** is cited by its `$id`, e.g.
  `urn:living-atlas:contract:2026.08.4:tool:atlas.assertion.query.v1:output`.
...
- **A tool document resolves alone.** Every `$ref` in a published tool document
  **MUST** [C-47] resolve within that document — a same-document pointer, or a URN
  naming a resource embedded under the document's own `$defs`. The wire hands a
  consumer one document per tool and protocol `2026-07-28` forbids it from
  dereferencing anything outside it, so a cross-document reference that is not
  carried inside the document is unresolvable for every conforming client.
...
again, and the bytes of `schema/2026.08.4/` will not either once it is frozen. This
revision itself is the rule applied: making 2026.08.3's tool documents self-contained
would have been a one-line serve-time change, and it shipped as a new directory
instead, because 2026.08.3's bytes are what its consumers fetched.
...
| A11 | Embedding the definitions a tool document's `$ref`s reach under its own `$defs` (JSON Schema bundling). No `$ref` string changes, the embedded copies are the published definitions, and no payload that validated before validates differently. |
...
| C-47 | Every $ref in a published tool document resolves within that document | `packages/atlas-contract/src/schema.test.ts` › "publishes every tool document self-contained: it compiles with nothing else registered" |
